 ...
 * @author Antz
 */
class Unbxd_Search_Helper_Data extends Mage_Core_Helper_Abstract
{
    /**
     * Allowed languages.
    ...
    /**
    * Boolean field which stores unbxd active or not 
    *
    * @var boolean
    */
...
//            $languageSuffix = "_fq";
    ...
    {   
        if(is_null($this->is_active)) {
            $config = $this->getEngineConfigData('search');
            $siteKey = $this->getSiteName();
            $apiKey = $this->getApiKey();
            if(isset($config) && isset($siteKey) && isset($apiKey)) {
                $this->is_active = (Mage::helper('unbxd_searchcore')->isConfigTrue(Mage::app()->getWebsite(), Unbxd_Searchcore_Helper_Constants::SEARCH_MOD_STATUS) &&
                    ($config[Unbxd_Searchcore_Helper_Constants::SEARCH_MOD_POWER] ==
                    Unbxd_Searchcore_Helper_Constants::ALL_POWER_LABEL ||
                    ((strtolower($config[Unbxd_Searchcore_Helper_Constants::SEARCH_MOD_POWER]) ==
                            Unbxd_Searchcore_Helper_Constants::NAVIGATION_POWER_LABEL)
                        && $this->isNavigation()) ||
                    ($this->isSearch() &&
                        strtolower($config[Unbxd_Searchcore_Helper_Constants::SEARCH_MOD_POWER]) ==
                        Unbxd_Searchcore_Helper_Constants::SEARCH_POWER_LABEL)));
            } else {
    ...
    public function getSiteName(){
        ...
        if(array_key_exists($siteKeyLabel, $config) && $config[$siteKeyLabel] != "")
            ...
            if(isset($siteKey) && $siteKey != ''){
                Mage::helper('unbxd_searchcore')->saveConfig(Mage::app()->getWebsite(),
    ...
    */
    ...
    public function isHostedSearchActive()
    {
        ...
        $hostedIntStatus = $searchConf[Unbxd_Searchcore_Helper_Constants::SEARCH_HOSTED_INT_STATUS];
        $hostedSearchRedirectUrl = $searchConf[Unbxd_Searchcore_Helper_Constants::SEARCH_HOSTED_REDIRECT_URL];
        return !is_null($siteKey) && !is_null($apiKey) &&
        $hostedSearchConf && !$searchModConf &&
        $hostedIntStatus == Unbxd_Searchcore_Helper_Constants::SEARCH_HOSTED_INT_COMPLETE &&
    ...
    public function getHostedRedirectUrl() {
        $config = $this->getEngineConfigData(Unbxd_Searchcore_Helper_Constants::SEARCH_HOSTED_REDIRECT_URL);
        return $config[Unbxd_Searchcore_Helper_Constants::SEARCH_HOSTED_REDIRECT_URL];
    }
 }
