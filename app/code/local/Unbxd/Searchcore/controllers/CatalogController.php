 ...
 */
class Unbxd_Searchcore_CatalogController extends Mage_Core_Controller_Front_Action {
    ...
    protected function _prepare() {
        ...
        if(!isset($website) || !$website->hasData("website_id")) {
            Mage::helper('unbxd_searchcore')->log(Zend_Log::DEBUG,'api failed because of invalid website');
    ...
    public function filterAction() {
        ...
        if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
            $filters = Mage::getResourceModel('unbxd_searchcore/config')->getFilters($website);
            ...
            foreach($requestBody as $key=>$value) {
                $filterValues[] = $key . Unbxd_Searchcore_Model_Config::FILTER_DELIMITER . $value;
            }
            Mage    ::getResourceModel('unbxd_searchcore/config')->updateValues($website->getWebsiteId(),
                Unbxd_Searchcore_Model_Config::FILTER, $filterValues);
            $this->getResponse()->setBody(json_encode(array('success' => true)));
        } else {
