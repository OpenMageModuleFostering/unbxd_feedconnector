 ...
 */
class Unbxd_Searchcore_ConfigController extends Mage_Core_Controller_Front_Action {

    /**
     * Searchcore
     * @return Unbxd_Searchcore_Helper_Confighelper
     */
    protected function _helper() {
        return Mage::helper("unbxd_searchcore/confighelper");
    }
    ...
    public function filterAction() {
        ...
        if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
            Mage::getResourceModel('unbxd_searchcore/config')->getValues($website->getWebsiteId(),
                Unbxd_Searchcore_Helper_Constants::FILTER);
    ...
    public function fieldsAction() {
        ...
        if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
            $mappedFields = Mage::getResourceModel('unbxd_searchcore/field')->getDisplayableFields($website)
                ->__asArray();
            $featureFields = Unbxd_Searchcore_Model_Field::$displayableFeatureFields;
            foreach($mappedFields as $fields) {
                if(array_key_exists(Unbxd_Searchcore_Model_Field::featured_field, $fields) &&
                    in_array($fields[Unbxd_Searchcore_Model_Field::featured_field], $featureFields)) {
                    unset($featureFields[array_search($fields[Unbxd_Searchcore_Model_Field::featured_field],
                        $featureFields)]);
                }
            }

            foreach($featureFields as $field) {
                $mappedFields[] = array(Unbxd_Searchcore_Model_Field::featured_field => $field,
                    Unbxd_Searchcore_Model_Field::datatype => 'text',
                    Unbxd_Searchcore_Model_Field::autosuggest => 1);
            }
            ...
            $this->getResponse()->setBody(json_encode(array('success' => true,
                ...
                'datatype' => Unbxd_Searchcore_Model_Field::$data_types)));
        } else if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::POST) {
    ...
    public function analyticsimpressionAction() {
        ...
        if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
            ...
            $analyticsResponse = Mage::getModel("unbxd_searchcore/api_task_analyticsimpression")
                ->prepare($website)
                ->process();
	    ...
	    $searchResponse = Mage::getSingleton('unbxd_searchcore/api_task_searchimpression')
                ->prepare($website)
                ->process();
    ...
    public function keysAction() {
        ...
        if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
            ...
            $siteKey = Mage::getResourceModel("unbxd_searchcore/config")
                ->getValue($website->getWebsiteId(), Unbxd_Searchcore_Helper_Constants::SITE_KEY);
            $errors = array();
            if(is_null($secretKey)) {
                $errors[Unbxd_Searchcore_Helper_Constants::SECRET_KEY] = "secret key not set";
            }
            if(is_null($siteKey)) {
                $errors[Unbxd_Searchcore_Helper_Constants::SITE_KEY] = "site key not set";
            }
            if(sizeof($errors) > 0) {
                $this->_helper()
            ...
            $this->getResponse()->setBody(json_encode(array('success' => true,
                Unbxd_Searchcore_Helper_Constants::SECRET_KEY => $secretKey,
                Unbxd_Searchcore_Helper_Constants::SITE_KEY => $siteKey)));
            return;
        } else if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::POST) {
    ...
    public function downloadAction() {
        $filepath = Mage::getBaseDir('log').DS.Unbxd_Searchcore_Helper_Data::LOG_FILE;
    ...
    public function unlockfeedAction() {
        ...
        Mage::getResourceModel('unbxd_searchcore/config')->unLockSite($website->getWebsiteId());
        $this->getResponse()->setBody(json_encode(array('success' => true)));
    }
    ...
    public function cronAction() {
        ...
        $isCronEnabled = Mage::getResourceSingleton('unbxd_searchcore/config')->getValue($website->getWebsiteId(),
            Unbxd_Searchcore_Helper_Constants::IS_CRON_ENABLED);
    ...
    public function supportmailAction() {
        ...
        if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::POST) {
            $request = $this->getRequest();
            $requestBody = $this->_getRawBody($request);
            if(!array_key_exists(Unbxd_Searchcore_Helper_Constants::CC, $requestBody) ||
                !array_key_exists(Unbxd_Searchcore_Helper_Constants::SUBJECT, $requestBody) ||
                    !array_key_exists(Unbxd_Searchcore_Helper_Constants::CONTENT, $requestBody)) {
                $this->getResponse()->setBody(json_encode(array('success' => false, 'message' => 'Invalid parameters')));
                return;
            }
            Mage::getModel('unbxd_searchcore/api_task_supportmail')
                ->setSubject($requestBody[Unbxd_Searchcore_Helper_Constants::SUBJECT])
                ->setCc($requestBody[Unbxd_Searchcore_Helper_Constants::CC])
                ->setContent($requestBody[Unbxd_Searchcore_Helper_Constants::CONTENT])
                ->prepare($website)
                ->process();
            $this->getResponse()->setBody(json_encode(array('success' => true)));
        } else {
    ...
    public function searchimpressionAction() {
        ...
        if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
            $response = Mage::getSingleton('unbxd_searchcore/api_task_searchimpression')
                ->prepare($website)
                ->process();
	...
	if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
	    $response = Mage::getResourceSingleton("unbxd_searchcore/config")->getValue($website->getWebsiteId(), 
						array(Unbxd_Searchcore_Model_Config::LAST_UPLOAD_TIME,
							Unbxd_Searchcore_Model_Config::FEED_STATUS));
	    if(!array_key_exists(Unbxd_Searchcore_Model_Config::LAST_UPLOAD_TIME, $response)) {
		$response[Unbxd_Searchcore_Model_Config::LAST_UPLOAD_TIME] = null;
		$response[Unbxd_Searchcore_Model_Config::FEED_STATUS] = 'NOT UPLOADED';
	    } 
            $this->getResponse()->setBody(json_encode(array('success' => true) + $response));
        } else {
            $this->getResponse()->setBody(json_encode(array('success' => false,
	...
	if($_SERVER['REQUEST_METHOD'] == Zend_Http_Client::GET) {
		$response = Mage::getModel('unbxd_searchcore/statemgr')->getAllStates($website);
		$this->getResponse()->setBody(json_encode(array('success' => true) + $response));
	} else {
		$this->getResponse()->setBody(json_encode(array('success' => false,
			'errors' => array('message' => 'Invalid method'))));
