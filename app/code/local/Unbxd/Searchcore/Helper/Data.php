 ...
 */
class Unbxd_Searchcore_Helper_Data extends Mage_Core_Helper_Abstract {

    const LOG_FILE = "unbxd_searchcore.log";
	...
	if($original) {
        ...
        if(is_null($this->engineConfigData)) {
            $this->engineConfigData = Mage::getConfig()
                ->getNode(Unbxd_Searchcore_Helper_Constants::UNBXD_CONFIG_PREFIX, 'websites',
    ...
    public function getOriginalValue($prefix = '', Mage_Core_Model_Website $website) {
       $configDataCollection = Mage::getModel('core/config_data')
                     ->getCollection()
                     ->addScopeFilter('websites', (int)$website->getWebsiteId(), Unbxd_Searchcore_Helper_Constants::UNBXD_CONFIG_PREFIX)
                     ->load();
	$value =array();
         foreach($configDataCollection as $data) {
                 $path = $data->getPath();
		 if (substr($path, 0, strlen(Unbxd_Searchcore_Helper_Constants::UNBXD_CONFIG_PREFIX)) == Unbxd_Searchcore_Helper_Constants::UNBXD_CONFIG_PREFIX) {
    			$path = substr($path, strlen(Unbxd_Searchcore_Helper_Constants::UNBXD_CONFIG_PREFIX) +1);
		}
	...
	if($configName == Unbxd_Searchcore_Helper_Constants::SEARCH_HOSTED_STATUS && $value == Unbxd_Searchcore_Helper_Constants::TRUE && $this->isConfigTrue($website, Unbxd_Searchcore_Helper_Constants::SEARCH_MOD_STATUS, $origData)) {
		return "Module search is on";
	}
	if($configName == Unbxd_Searchcore_Helper_Constants::SEARCH_MOD_STATUS && $value == Unbxd_Searchcore_Helper_Constants::TRUE && $this->isConfigTrue($website, Unbxd_Searchcore_Helper_Constants::SEARCH_HOSTED_STATUS, $origData)) {
    ...
    public function saveConfig(Mage_Core_Model_Website $website, $data) {
        foreach($data as $key => $value) {
            if(!is_null($this->isConfigSaveAllowed($website, $key, $value, true)))
                continue;
            Mage::getConfig()
                ->saveConfig(Unbxd_Searchcore_Helper_Constants::UNBXD_CONFIG_PREFIX .
    ...
    public function isConfigTrue(Mage_Core_Model_Website $website, $configName, $origData = false) {
        ...
        return array_key_exists($configName, $configData) &&
            $configData[$configName] == Unbxd_Searchcore_Helper_Constants::TRUE;
    }
    ...
    */
    public function getSiteName(){
        $siteKeyLabel = Unbxd_Searchcore_Helper_Constants::SITE_KEY;
        ...
        if(array_key_exists($siteKeyLabel, $config) && $config[$siteKeyLabel] != "")
            ...
            if(isset($siteKey) && $siteKey != ''){
                ...
                if(Mage::getIsDeveloperMode()) {
                    ...
                    Mage::helper('unbxd_searchcore')->log(Zend_Log::ERR, 'Unbxd site key is not set');
                    return null;
                }
            }
        }
    ...
    */
    public function getApiKey(){
        $apiKeyLabel = Unbxd_Searchcore_Helper_Constants::API_KEY;
            ...
            if(isset($apiKey) && $apiKey != ''){
               $this->saveConfig(Mage::app()->getWebsite(),
                ...
                if(Mage::getIsDeveloperMode()) {
                    ...
                    Mage::helper('unbxd_searchcore')->log(Zend_Log::ERR, 'Unbxd Api key is not set');
                    return null;
                }
            }
        }
    ...
    /*
     ...
     public function isExecutable() {
         if(Mage::helper('core')->isModuleEnabled('Unbxd_Recscore')){
                 $otherModuleVersion = floatval(Mage::getConfig()->getModuleConfig("Unbxd_Recscore")->version);
                 $currentModuleVersion = floatval(Mage::getConfig()->getModuleConfig("Unbxd_Searchcore")->version);
