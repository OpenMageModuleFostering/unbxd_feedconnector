...
class Unbxd_Searchcore_Helper_Feedhelper extends Unbxd_Searchcore_Helper_Data {
    ...
    public function isMultiSelect($attributeName = ""){
        ...
        if( $this->isMultiSelectDatatype($attributeName)||
            ...
            $attributeName == Unbxd_Searchcore_Model_Resource_Field::CAT_LEVEL_2_NAME ||
            $attributeName == Unbxd_Searchcore_Model_Resource_Field::CAT_LEVEL_3_NAME ||
    ...
    public function getFilters(Mage_Core_Model_Website $website) {
        if(!isset($this->_filters)) {
            $this->_filters = Mage::getResourceModel('unbxd_searchcore/config')->getFilters($website);
        }
