 ...
 */
class Unbxd_Searchcore_Model_Resource_Field extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    {
        $this->_init('unbxd_searchcore/field', 'id');
        $this->_unbxdFieldTable = $this->getTable('unbxd_searchcore/field');
    }
    ...
    public function getFieldByFeatureField($websiteId, $featureField){
        ...
        $select = $adapter->select()
            ->from($this->_unbxdFieldTable, Unbxd_Searchcore_Model_Field::field_name)
            ->where('`'.Unbxd_Searchcore_Model_Field::website_id.'` = ?', (int)$websiteId)
            ->where('`'.Unbxd_Searchcore_Model_Field::featured_field.'` = ?', $featureField);
    ...
    protected function getDisplayableFieldCollection(Mage_Core_Model_Website $website) {
        return Mage::getModel("unbxd_searchcore/field")
            ->getCollection()
