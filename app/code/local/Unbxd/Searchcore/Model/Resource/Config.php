 ...
 */
class Unbxd_Searchcore_Model_Resource_Config extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    {
        $this->_init('unbxd_searchcore/config', 'id');
        $this->_unbxdConfigTable = $this->getTable('unbxd_searchcore/config');
    }
    ...
    public function getValues($websiteId, $key) {
        ...
        $values = array();
        foreach($rows as $row) {
            if(array_key_exists(Unbxd_Searchcore_Model_Config::VALUE, $row)) {
                $values[] = $row[Unbxd_Searchcore_Model_Config::VALUE];
            }
        }
        return $values;
    }
    ...
    public function getValue($website_id, $key)
    {
        ...
        $select = $adapter->select()
            ...
            ->where('`'.Unbxd_Searchcore_Model_Config::WEBSITE_ID.'` = ?', (int)$website_id)
            ->where('`'.Unbxd_Searchcore_Model_Config::KEY.'` = ?', $key);
    ...
    public function lockSite($website_id) {
        $this->setValue($website_id, array(
            Unbxd_Searchcore_Model_Config::FEED_LOCK => Unbxd_Searchcore_Model_Config::FEED_LOCK_TRUE,
            Unbxd_Searchcore_Model_Config::FEED_LOCK_TIME => date(self::DATE_FORMAT)));

    }


    public function unLockSite($website_id) {
        $this->setValue($website_id, Unbxd_Searchcore_Model_Config::FEED_LOCK,
            Unbxd_Searchcore_Model_Config::FEED_LOCK_FALSE);
    }
    ...
    public function isLock($website_id) {
        ...
        $feedLockDetails = $this->getValue($website_id,
            array(Unbxd_Searchcore_Model_Config::FEED_LOCK, Unbxd_Searchcore_Model_Config::FEED_LOCK_TIME));
        //fetch feed lock from @var feedLockDetails
        $feedLock = array_key_exists(Unbxd_Searchcore_Model_Config::FEED_LOCK, $feedLockDetails)?
            $feedLockDetails[Unbxd_Searchcore_Model_Config::FEED_LOCK]:null;
            if(is_null($feedLock) || $feedLock == Unbxd_Searchcore_Model_Config::FEED_LOCK_FALSE){
            return false;
        }
        // Ignoring the feed Lock, if the feed has been locked more than $maxFeedLockTime
        if($feedLock == Unbxd_Searchcore_Model_Config::FEED_LOCK_TRUE &&
            array_key_exists(Unbxd_Searchcore_Model_Config::FEED_LOCK_TIME, $feedLockDetails)) {
            $feedLockTime  = $feedLockDetails[Unbxd_Searchcore_Model_Config::FEED_LOCK_TIME];
            ...
            if(is_null($maxFeedLockTime)) {
                $maxFeedLockTime = Unbxd_Searchcore_Model_Config::MAX_FEED_LOCK_TIME;
            }
    ...
    public function getFilters(Mage_Core_Model_Website $website) {
        $values = Mage::getResourceModel('unbxd_searchcore/config')->getValues($website->getWebsiteId(),
            Unbxd_Searchcore_Model_Config::FILTER);
        $filters = array();
        foreach($values as $value) {
            $explodedValues = explode(Unbxd_Searchcore_Model_Config::FILTER_DELIMITER, $value);
