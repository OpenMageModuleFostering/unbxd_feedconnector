 ...
 */
class Unbxd_Searchcore_Model_Observer {
	...
	if(!Mage::helper('unbxd_searchcore')->isExecutable()) {
		return;
	}
        $product = $observer->getEvent()->getProduct();
        if(!$product instanceof Mage_Catalog_Model_Product) {
            Mage::helper('unbxd_searchcore')->log(Zend_Log::ERR, 'CART_TRACKER:product is not a valid type');
            return $this;
        }
	$uniqueId = Mage::helper('unbxd_searchcore/feedhelper')->getUniqueId($product);
        $response = Mage::getModel('unbxd_searchcore/api_task_trackcart')
            ...
            ->process();
	Mage::helper('unbxd_searchcore')->log(Zend_Log::DEBUG, "CART_TRACKER: request with uniqueId ".$uniqueId);
	...
	if(!Mage::helper('unbxd_searchcore')->isExecutable()) {
        ...
        if(!$payment instanceof Mage_Sales_Model_Order_Payment) {
            Mage::helper('unbxd_searchcore')->log(Zend_Log::ERR, 'ORDER_TRACKER:payment is not a valid type');
            return $this;
        }
	    ...
	    $uniqueId = Mage::helper('unbxd_searchcore/feedhelper')->getUniqueId($product, $item);
            $response = Mage::getModel('unbxd_searchcore/api_task_trackorder')
                ->setData('data',
                ...
                ->prepare(Mage::app()->getWebsite())
                ->process();
	    Mage::helper('unbxd_searchcore')->log(Zend_Log::DEBUG, "ORDER_TRACKER: request with uniqueId ".$uniqueId);

	    if(!$response->isSuccess()) {
                Mage::helper('unbxd_searchcore')
                    ->log(Zend_Log::ERR, 'ORDER_TRACKER:request failed because ' . json_encode($response->getErrors()));
            }
            Mage::getSingleton('unbxd_searchcore/sync')->addProduct($product);
        }
        return $this;
	}
	...
	if(!Mage::helper('unbxd_searchcore')->isExecutable()) {
        ...
        foreach ($websiteCollection as $website) {
            ...
            Mage::getResourceModel('unbxd_searchcore/config')
                ->setValue($website->getWebsiteId(), Unbxd_Searchcore_Helper_Constants::IS_CRON_ENABLED, 1);
            Mage::getSingleton('unbxd_searchcore/feed_feedmanager')->process(true, $website);
        }
        return $this;
    }
    ...
    public function syncIncremental($observer)
    {
        ...
        foreach ($websiteCollection as $website) {
            ...
            Mage::getResourceModel('unbxd_searchcore/config')
               ->setValue($website->getWebsiteId(), Unbxd_Searchcore_Helper_Constants::IS_CRON_ENABLED, 1);
            Mage::getSingleton('unbxd_searchcore/feed_feedmanager')->process(false, $website);
       }
       return $this;
   }
    ...
    public function trackDelete(Varien_Event_Observer $observer) {
        ...
        foreach($parentIds as $parentId) {
            ...
            Mage::getSingleton('unbxd_searchcore/sync')->addProduct($parentProduct);
        }
        Mage::getSingleton('unbxd_searchcore/sync')->deleteProduct($product);
        return $this;
    }
