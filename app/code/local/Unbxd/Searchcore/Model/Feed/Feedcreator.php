...
class Unbxd_Searchcore_Model_Feed_Feedcreator {
    ...
    public function setConfig(Unbxd_Searchcore_Model_Feed_Feedconfig $config) {
    ...
    protected  function _writeSchemaContent() {
        return $this->_appendTofile('"schema":'.
            Mage::getSingleton('unbxd_searchcore/feed_jsonbuilder_schemabuilder')->getSchema($this->fields));
    }
    ...
    protected  function _getCatalogCollectionToAdd(Mage_Core_Model_Website $website, $fromDate, $currentDate) {
        if($this->isFullUpload()) {
            return Mage::getResourceModel('unbxd_searchcore/product_collection')
                ->addFullUploadFilters($website);
        } else {
            $products = Mage::getModel('unbxd_searchcore/sync')
                ->getCollection()
                ->addWebsiteFilter($website->getWebsiteId())
                ->addUnsyncFilter()
                ->addOperationFilter(Unbxd_Searchcore_Model_Sync::OPERATION_ADD)
                ->load();
            $productIds = array();
    ...
    protected function _getCatalogCollectionToDelete(Mage_Core_Model_Website $website) {
        $products = Mage::getModel('unbxd_searchcore/sync')
            ...
            ->addWebsiteFilter($website->getWebsiteId())
            ->addUnsyncFilter()
            ->addOperationFilter(Unbxd_Searchcore_Model_Sync::OPERATION_DELETE)
            ->load();
        $collection = Mage::getResourceModel('unbxd_searchcore/product_collection');
    ...
    protected function _writeDeleteProductsContent(Mage_Core_Model_Website $website, $fromDate, $currentDate) {
        ...
        $collection2 = Mage::getResourceModel('unbxd_searchcore/product_collection')
        ...
        $collection = $collection1
            ...
            ->virtuallyLoad();
        return $this->_writeProducts($website, $collection, Unbxd_Searchcore_Model_Feed_Tags::DELETE, true);
    }
    ...
    protected function _processCollection($collection , $pageNum,
                                          $operation = Unbxd_Searchcore_Model_Feed_Tags::ADD, $loadAll = false) {
        if(!$loadAll) {
            $collection->clear();
    ...
    protected function _writeProducts(Mage_Core_Model_Website $website, $collection,
                                      $operation = Unbxd_Searchcore_Model_Feed_Tags::ADD, $loadAllAtOnce = false)
    {
        $pageNum = $this->page;
        ...
        while (true) {
            ...
            if (!$firstLoop && $loadAllAtOnce) {
                ...
                if (!$this->_appendTofile(Unbxd_Searchcore_Model_Feed_Tags::COMMA)) {
                ...
                if (!$this->_appendTofile(
                    Mage::getSingleton('unbxd_searchcore/feed_tags')->getKey($operation) .
                    Unbxd_Searchcore_Model_Feed_Tags::COLON . Unbxd_Searchcore_Model_Feed_Tags::OBJ_START .
                    Unbxd_Searchcore_Model_Feed_Tags::DOUBLE_QUOTE .
                    Unbxd_Searchcore_Model_Feed_Tags::ITEMS .
                    Unbxd_Searchcore_Model_Feed_Tags::DOUBLE_QUOTE .
                    Unbxd_Searchcore_Model_Feed_Tags::COLON . Unbxd_Searchcore_Model_Feed_Tags::ARRAY_START)
                ) {
                    $this->log("Error while adding items tag");
                    return false;
                }
            }
            $content = Mage::getSingleton('unbxd_searchcore/feed_jsonbuilder_productbuilder')
        ...
        if (!$this->_appendTofile(Unbxd_Searchcore_Model_Feed_Tags::ARRAY_END .
            Unbxd_Searchcore_Model_Feed_Tags::OBJ_END)
        ) {
    ...
    protected function _setCopyFields(Mage_Core_Model_Website $website) {
        ...
        $copyFields = Mage::getModel('unbxd_searchcore/field')->getCopyFields($website);
        foreach($copyFields as $fieldName => $copyField) {
            if(array_key_exists($copyField, $this->fields)) {
                $this->_copyFields[$fieldName] = $copyField;
            }
        }
        $this->_copyFields[Mage::getModel('unbxd_searchcore/field')->getImageUrlFieldName()] = "imageUrl";
    }


    protected function _setFields(Mage_Core_Model_Website $website) {
        $fields = Mage::getResourceModel("unbxd_searchcore/field_collection")->getFields($website);
        ...
        foreach($fields as $eachfield) {
            ...
            if(!is_null($eachfield->getFeaturedField()) &&
                ...
                if($eachfield->getFeaturedField() == "imageUrl") {
                    ...
                    $this->fields[$eachfield->getFieldName()] = Mage::getModel('unbxd_searchcore/field')->getField('longText', "false", "false");
                    $this->fields["imageUrl"][Unbxd_Searchcore_Model_Feed_Jsonbuilder_Productbuilder::GENERATE_IMAGE] = "1";
                } else {
            ...
            $field = array();
            $field[Unbxd_Searchcore_Model_Field::status] = 1;
            $field[Unbxd_Searchcore_Model_Field::datatype] = $eachfield->getDatatype();
            $field[Unbxd_Searchcore_Model_Field::autosuggest] = $eachfield->getAutosuggest();
            $field[Unbxd_Searchcore_Model_Field::multivalued] = Mage::helper('unbxd_searchcore/feedhelper')
    ...
    protected function _setImageConf(Mage_Core_Model_Website $website) {
        $imageFields = Mage::getModel('unbxd_searchcore/field')->getImageFields($website);
        foreach($this->fields as $fieldName => $fieldConf) {
            if(array_key_exists($fieldName, $imageFields)) {
                $this->fields[$fieldName][Unbxd_Searchcore_Model_Feed_Jsonbuilder_Productbuilder::GENERATE_IMAGE] = '1';
            }
        }
    }
    ...
    protected  function _createFile(){
        return Mage::getSingleton('unbxd_searchcore/feed_filemanager')->createFile($this->fileName);
    }

    protected function _appendTofile($data){
        return Mage::getSingleton('unbxd_searchcore/feed_filemanager')->appendTofile($this->fileName, $data);
    }
