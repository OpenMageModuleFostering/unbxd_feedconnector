...
class Unbxd_Searchcore_Model_Feed_Feedmanager {
	...
	$feedUrl = Mage::helper('unbxd_searchcore/confighelper')->getConfigData('feed_url');
	...
	/**
	 ...
	 protected function _checkFeedUpload(Mage_Core_Model_Website $website) {
		$lastUploadTime = Mage::getResourceModel('unbxd_searchcore/config')
				->getValue($website->getWebsiteId(), Unbxd_Searchcore_Helper_Constants::LAST_UPLOAD_TIME);
		return !is_null($lastUploadTime);
	 }
	protected function _triggerSearchComplete(Mage_Core_Model_Website $website) {
		Mage::helper('unbxd_searchcore')->log(Zend_Log::DEBUG, 'trigger search complete for website ' . $website->getName());
	...
	**/
	public function init(Mage_Core_Model_Website $website, $isFullUpload)
	{
        ...
        $this->key = Mage::getResourceModel("unbxd_searchcore/config")
            ->getValue($website->getWebsiteId(), Unbxd_Searchcore_Helper_Confighelper::SECRET_KEY);
        $this->siteName = Mage::getResourceModel("unbxd_searchcore/config")
		...
		if(is_null($this->siteName) || is_null($this->key)) {
            ...
            Mage::helper('unbxd_searchcore')->log(Zend_Log::ERR, $message);
    ...
    private function updateFeatureFields(Mage_Core_Model_Website $website) {
        $needFeatureFieldUpdation = Mage::getResourceModel("unbxd_searchcore/config")
            ->getValue($website->getWebsiteId(), Unbxd_Searchcore_Helper_Confighelper::NEED_FEATURE_FIELD_UPDATION);
        if(is_null($needFeatureFieldUpdation) || $needFeatureFieldUpdation == "1") {
            $response = Mage::helper('unbxd_searchcore/confighelper')->updateFeatureFields($website);
            if($response) {
		...
		Mage::helper('unbxd_searchcore/confighelper')->triggerAutoggestIndexing($website);
    ...
    public function process($isFullUpload = true, Mage_Core_Model_Website $website)
    {
        ...
        if (!$isFullUpload ||
            !Mage::getResourceModel("unbxd_searchcore/config")->isLock($website->getWebsiteId())
        ) {

            $this->log('site ' . $website->getName() . ' is acquiring feed lock');
            if ($isFullUpload) {
                Mage::getResourceSingleton("unbxd_searchcore/config")
                    ->setValue($website->getWebsiteId(),
                        Unbxd_Searchcore_Model_Config::FEED_STATUS,
                        Unbxd_Searchcore_Helper_Constants::FEED_STATUS_UPLOADING);
                Mage::getResourceModel('unbxd_searchcore/config')->lockSite($website->getWebsiteId());
            }
            try {
                // create the feed
            ...
            if ($status) {
                ...
                if ($status) {
                    Mage::getResourceSingleton("unbxd_searchcore/config")
                        ->setValue($website->getWebsiteId(),
                            Unbxd_Searchcore_Model_Config::LAST_UPLOAD_TIME, $currentDate);
            ...
            if ($isFullUpload) {
                ...
                // Mage::getSingleton('unbxd_searchcore/filemanager')->deleteFile($this->fileName);
            }
            ...
            if ($status) {
                Mage::getResourceSingleton("unbxd_searchcore/config")
                    ->setValue($website->getWebsiteId(),
                        Unbxd_Searchcore_Model_Config::FEED_STATUS,
                        Unbxd_Searchcore_Helper_Constants::FEED_STATUS_UPLOADED_SUCCESSFULLY);
                return array('success' => true, 'message' => 'File uploaded successfully');
            }
            Mage::getResourceSingleton("unbxd_searchcore/config")
                ->setValue($website->getWebsiteId(),
                    Unbxd_Searchcore_Model_Config::FEED_STATUS,
                    Unbxd_Searchcore_Helper_Constants::FEED_STATUS_UPLOADED_FAILED);
