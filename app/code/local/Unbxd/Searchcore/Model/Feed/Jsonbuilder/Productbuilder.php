...
class Unbxd_Searchcore_Model_Feed_Jsonbuilder_Productbuilder extends
	Unbxd_Searchcore_Model_Feed_Jsonbuilder_Jsonbuilder {
	...
	/**
	 * @return Unbxd_Searchcore_Helper_Feedhelper
	 */
	protected function _getFeedHelper() {
		return Mage::helper('unbxd_searchcore/feedhelper');
	}
	...
	public function getProducts($website, $collection, $fields, $copyFields) {
		...
		self::$CATEGORY_EXCLUSION_LIST = Mage::helper('unbxd_searchcore/confighelper')->getCategoryExclusion($website);
		$content='';
		$firstLoop = true;
	...
	public function skipProduct(Mage_Core_Model_Website $website, $product) {
		...
		foreach($filters as $key=>$filter) {
			if($this->_getFeedHelper()->isMultiSelect($key)) {
				...
				$temp = explode(Unbxd_Searchcore_Helper_Constants::FILTER_RANGE_DELIMITER,$filter);
				if(sizeof($temp) >=2) {
					$from = int($temp[0]);
					$to = int($temp[1]);
				} else {
	...
	protected function _getCategoryAttribute($product){
		...
		$categoryData[Unbxd_Searchcore_Model_Resource_Field::CATEGORY_IDS_NAME] = $categoryIds;
		$categoryData[Unbxd_Searchcore_Model_Resource_Field::CATEGORY_NAME] = array_values(array_unique(array_diff($category, self::$CATEGORY_EXCLUSION_LIST)));
	...
	protected function _getMultiSelectAttribute($fieldName, $product) {
		...
		foreach($data as $eachdata){
			$attributeValue = Mage::getResourceSingleton("unbxd_searchcore/attribute")
	...
	*/
	private function renameConflictedFeatureFields($unbxdFieldName) {
		if (in_array($unbxdFieldName, Mage::getResourceSingleton('unbxd_searchcore/field')->getConflictedFeatureFieldLust())) {
	...
	*/
	private function convertDataTypeByValue($data_type, $value) {
		if($data_type[Unbxd_Searchcore_Model_Field::datatype] == self::DECIMAL) {
			return $this->getFloatValues($value);
		} else if ($data_type[Unbxd_Searchcore_Model_Field::datatype] == self::NUMBER) {
			return $this->getNumberValues($value);
		} else if ($data_type[Unbxd_Searchcore_Model_Field::datatype] == self::DATE) {
		...
		foreach($product as $field=>$value) {
			if((is_null($fields) ||
					(($field != "associatedProducts") &&
						array_key_exists(Unbxd_Searchcore_Model_Field::multivalued,$fields[$field]) &&
						$fields[$field][Unbxd_Searchcore_Model_Field::multivalued])) &&
				!is_array($value)) {
