 ...
 */
class Unbxd_Searchcore_Model_Sync extends Mage_Core_Model_Abstract {
    ...
    public function addProduct(Mage_Catalog_Model_Product $product = null) {
        if (is_null($product) || !$product->hasData('entity_id')) {
            Mage::helper('unbxd_searchcore')->log(Zend_Log::ERR, 'product argument sent is empty');
            return false;
        }
        $write = Mage::getSingleton("core/resource")->getConnection("core_write");
        foreach($product->getWebsiteIds() as $websiteId) {

            $query = "insert into " . $this->getResource()->getTable('unbxd_searchcore/sync')
    ...
    public function markItSynced($websiteId, $toTime) {
        $write = Mage::getSingleton("core/resource")->getConnection("core_write");
        $query = "update " . $this->getResource()->getTable('unbxd_searchcore/sync')
