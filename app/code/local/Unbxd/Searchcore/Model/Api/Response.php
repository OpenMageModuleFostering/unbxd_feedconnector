 ...
 */
class Unbxd_Searchcore_Model_Api_Response extends Varien_Object {
    ...
    public function setResponse(Zend_Http_Response $response, $url) {
        if($response->isSuccessful()) {
            ...
            if(!$this->isJsonResponse()) {
                #Mage::helper('unbxd_searchcore')->log(Zend_Log::DEBUG, "response from the " . $url . " is successful");
                $this->setData("body", $body);
                return $this;
            }
            $this->response =  json_decode($body, true);
            #Mage::helper('unbxd_searchcore')->log(Zend_Log::DEBUG, "response from the " . $url . " is " . $body);
            if($this->response == false || !is_array($this->response) || sizeof($this->response) == 0) {
                $this->success = false;
                $this->setMessage("Invalid Authorization credentials");
                Mage::helper('unbxd_searchcore')->log(Zend_Log::ERR, $url . " api failed cos ". $this->getMessage());
                return $this;
            }
        } else {
