 ...
 */
class Unbxd_Searchcore_Model_Api_Task_Analyticsimpression extends Unbxd_Searchcore_Model_Api_Task
{
    ...
    protected function prepareHeaders(Mage_Core_Model_Website $website) {
        $username = Mage::getResourceModel("unbxd_searchcore/config")
            ->getValue($website->getWebsiteId(), Unbxd_Searchcore_Helper_Confighelper::USERNAME);
        if(is_null($username)) {
            $this->preparationSuccessful = false;
