 ...
 */
class Unbxd_Searchcore_Model_Api_Task_Updatefeaturefields extends Unbxd_Searchcore_Model_Api_Task {
    ...
    protected function prepareData($website) {
        $featureFields = Mage::getResourceModel("unbxd_searchcore/field_collection")->getFeatureFields($website);
        $featureFieldMap = array();
        foreach($featureFields as $field) {
	   if($field[Unbxd_Searchcore_Model_Field::featured_field] == Unbxd_Searchcore_Helper_Constants::FEATURE_FIELD_IMAGE_URL) {
		$featureFieldMap[$field[Unbxd_Searchcore_Model_Field::featured_field]] = "unbxd_NA";
		continue;
	   }
            $featureFieldMap[$field[Unbxd_Searchcore_Model_Field::featured_field]] =
